    ...
    while current := parent.get(current):
        ...
        for d in map(lambda o: (current.d + o) & 3, [1, -1]):
            ...
            if (
                ...
                trace.update(equal_cost_backtrace(
                    parent, node_cost, parent.get(test)))
...
def a_star() -> tuple[int, int]:
    ...
    while len(pending):
        ...
        for i, vector in enumerate([(0, 1), (1, 0), (0, -1), (-1, 0)]):
            neighbor = Node(
                Point(current.p.y + vector[0], current.p.x + vector[1]), i)
            if board[neighbor.p.y][neighbor.p.x] == "#":
                continue

            new_cost = (
                node_cost[current] + [0, 1000, 2000,
                                      1000][(i - current.d) & 3] + 1
            )
